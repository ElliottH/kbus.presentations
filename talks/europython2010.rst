...
.. note:: There are various restrictions on what Limpets can do and how they
   must be used. See the online documentation for more information.

   Also, KBUS itself does not provide code for the communication between
   Limpets, since it does not mandate what mechanism is actually used,
   although a simple example using sockets is provided.
